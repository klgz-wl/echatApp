...
object NetworkModule {
    @Provides @Singleton fun paymentStorage(value: com.zorv.core.payment.PreferencePaymentStorage): com.zorv.core.payment.PaymentStorage = value
    @Provides @Singleton fun paymentEvents(value: com.zorv.core.payment.PaymentEventQueue): com.zorv.core.payment.PaymentEventSink = value
    @Provides @Singleton fun paymentClock() = com.zorv.core.payment.PaymentClock { System.currentTimeMillis() }
    @Provides @Singleton fun paymentOrders(wallet: com.zorv.core.wallet.WalletRepository, config: com.zorv.core.billing.BillingConfiguration) =
        com.zorv.core.payment.PaymentOrderFactory { productId, session -> wallet.createOrder(productId, config.defaultTrigger, session).orderId }

    @Provides @Singleton fun paymentRepository(value: com.zorv.core.payment.ApiPaymentRepository): com.zorv.core.payment.PaymentRepository = value
    @Provides @Singleton fun paymentApi(@Named("publicClient") client: OkHttpClient, sessions: SessionCoordinator,
        api: PublicAuthApi, config: CoreRuntimeConfig, identity: ClientIdentity, json: Json): com.zorv.core.payment.PaymentServiceApi {
        ...
        return retrofit(paymentClient, config, json).create(com.zorv.core.payment.PaymentServiceApi::class.java)
    }
    ...
    @Provides @Singleton fun attributionReportApi(@Named("publicClient") client: OkHttpClient, config: CoreRuntimeConfig, json: Json): com.zorv.core.attribution.AttributionReportApi =
        retrofit(client, config, json).create(com.zorv.core.attribution.AttributionReportApi::class.java)
    @Provides @Singleton fun eventApi(@Named("publicClient") client: OkHttpClient, config: CoreRuntimeConfig, json: Json): com.zorv.core.analytics.EventApi =
        retrofit(client, config, json).create(com.zorv.core.analytics.EventApi::class.java)
    ...
    @Provides @Singleton fun uploadedPhotoApi(retrofit: Retrofit): com.zorv.core.visual.UploadedPhotoApi = retrofit.create(com.zorv.core.visual.UploadedPhotoApi::class.java)
    @Provides @Singleton fun photoDownloadApi(identity: ClientIdentity, config: CoreRuntimeConfig, json: Json): com.zorv.core.visual.UploadedPhotoDownloadApi =
        retrofit(OkHttpClient.Builder().connectTimeout(identity.timeoutSeconds, TimeUnit.SECONDS)
            ...
            .create(com.zorv.core.visual.UploadedPhotoDownloadApi::class.java)
    @Provides @Singleton fun visualApi(retrofit: Retrofit): com.zorv.core.visual.VisualGenerationApi = retrofit.create(com.zorv.core.visual.VisualGenerationApi::class.java)
    @Provides @Singleton fun walletApi(client: OkHttpClient, config: CoreRuntimeConfig, json: Json): com.zorv.core.wallet.WalletApi =
        retrofit(client.newBuilder().retryOnConnectionFailure(false).followRedirects(false).followSslRedirects(false).build(), config, json)
            .create(com.zorv.core.wallet.WalletApi::class.java)
