...
class AttributionReportsTest {
    @Test fun `迟到归因补报后刷新资料切换模式且补报或资料失败均重试`() = runBlocking {
        ...
        profiles.refresh()
        assertEquals(com.zorv.core.config.AppMode.A, profiles.state.value?.mode)
        ...
        withTimeout(2000) { while (profiles.state.value?.mode != com.zorv.core.config.AppMode.B) delay(5) }
