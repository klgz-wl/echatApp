...
- Before completion, run `./gradlew --no-configuration-cache :core:testReleaseUnitTest :integration:analytics-appsflyer:testReleaseUnitTest :integration:analytics-thinkingdata:testReleaseUnitTest :app:testDevReleaseUnitTest :app:testProdReleaseUnitTest :app:assembleDevDebug :app:assembleDevRelease :app:lintDevRelease :app:assembleProdDebug`. The app variant tasks require the local sharedDev attachment; before it is available, `-x :app:verifyDevConfiguration -x :app:verifyProdConfiguration` may be used only for source/matrix diagnostics and must be reported as unsigned/unverified. Run `:app:connectedDevDebugAndroidTest` and `:core:connectedDebugAndroidTest` when a compatible emulator or device is available and UI behavior changed.
