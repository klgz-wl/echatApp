...
import yumo.achat.core.catalog.*
...
class GenerationRequestRepositoryTest {
    ...
    private suspend fun fixture(events: yumo.achat.core.analytics.EventTracker = yumo.achat.core.analytics.NoOpEventTracker): Fixture {
    ...
    @Test fun `上传受理与生成终态分开记录且恢复轮询不重复终态`() = runBlocking {
        val events = yumo.achat.core.analytics.RecordingTracker(); val f = fixture(events)
